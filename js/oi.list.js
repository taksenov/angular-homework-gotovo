    ...
    return function (scope, url, Resource, generate, options) {
      ...
      if (angular.equals(dataRes, [])) {
        scope.items = dataRes = oiListCache.cacheRes[url] = Resource.query(function(data) {
          ...
          setError(data.data.error, {msg: 'Невозможно загрузить список. Нет соединения с интернетом 001', code: 'load'});
        })
      ...
      function save (index) {
        if (typeof dataRes[index].id !== 'undefined') dataRes[index].$save(null, null, function (data) {
          setError(data.data.error, {msg: 'Невозможно сохранить элемент. Нет соединения с интернетом 002', code: 'save'}, dataRes[newIndex]);
        });
      }
        ...
        Resource.add(function (data) {
          ...
          if (typeof afterAdd !== 'function') {
            dataRes[newIndex].$save(null, null, function (data) {
              setError(data.data.error, {msg: 'Невозможно сохранить элемент. Нет соединения с интернетом 003', code: 'save'}, dataRes[newIndex]);
            });
          } else {
          ...
          setError(data.data.error, {msg: 'Невозможно создать элемент. Нет соединения с интернетом 004', code: 'add'});
      ...
      function del (index, beforeDel) {
        ...
        if (dataRes[index] !== undefined && dataRes[index].id !== undefined) { 
          ...
          dataRes[index].$remove(function (data) {
            ...
            delCount--
            setError(data.data.error, {msg: 'Невозможно удалить элемент. Нет соединения с интернетом 005', code: 'del'}, delItem);
        ...
        if (typeof generate === 'string' && !isEmpty(dataRes[last])) {
          ...
          Resource.add(function (data) {
            ...
            dataRes[prelast].$save(null, null, function (data) {
                setError(data.data.error, {msg: 'Невозможно сохранить элемент. Нет соединения с интернетом 006', code: 'save'}, dataRes[prelast]);
            });
            sort();
          }, function (data) {
            setError(data.data.error, {msg: 'Невозможно создать элемент. Нет соединения с интернетом 007', code: 'add'});
          })
        }
        ...
        if (typeof generate === 'string' && isEmpty(dataRes[prelast]) && typeof dataRes[prelast].id !== 'undefined') {                
          ...
          if (dataRes[prelast] !== undefined) {
            ...
            dataRes[last].$remove(function (data) {
              ...
              setError(data.data.error, {msg: 'Невозможно удалить элемент. Нет соединения с интернетом 008', code: 'del'}, dataRes[last]);
            });
          }
        }
      }
      ...
      function sort () {
        ...
        Resource.save(null, {sort: sortArr, pids: pidsArr}, null, function (data) {
          setError(data.data.error, {msg: 'Невозможно сохранить расположение элементов. Нет соединения с интернетом 009', code: 'sort'});
        })   
